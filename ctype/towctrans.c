...
const static union {
	unsigned char b[PAGEH + 131*8];
	unsigned int  w[PAGEH/4 + 131*2];
} table = {{
#include "towctrans_table.h"
}};

const static short dict[115] = {
...
static int casemap(unsigned wc, int dir)
{
	...
	/* Sum the run length prediction + residual in popc exceptions array
	   Runs are in 4 lane bi-endian SWAR format: stop|start|start|stop  */
